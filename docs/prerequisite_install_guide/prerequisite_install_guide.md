...
This guide will walk you through all of the software that needs to be installed and configured in order to contribute to the HexaDot project.
...
- [Getting the Source Code](#getting-the-source-code)
  ...
  - [Setup a link to the UDLab/HexaDot repository](#setup-a-link-to-the-udlabhexadot-repository)
...
>**Note:** the --global is used in the command below to set the default configuration for all repositories.  If it is left off, you need to first clone the repository and then run the command in the HexaDot folder to save it as just a project configuration.
...
HexaDot uses Visual Studio 2015 along with .NET Core.  This guide will be using the free Visual Studio 2015 Community Edition but any version of Visual Studio 2015 will work and the steps are the same for all versions.  
...
HexaDot uses ASP.NET Core and we need to install the tooling for .NET Core.  
...
1. Then run the following command to create an HexaDot directory and download the repository onto your machine.  Make sure to replace "[Your UserName]" with your actual Github user name.

        git clone http://github.com/[Your UserName]/HexaDot

You now have the repository on your machine.  

There are 2 Visual Studio solutions in the HexaDot\HexaDotApp directory.  

1. HexaDotWebonly.sln -> This file will open just the web project files and does not include the mobile application.  Use this solution if you are not going to be working on the mobile application. 
1. HexaDotApp.sln -> This file will open all of the HexaDot projects including the mobile application.  Use this file if you are going to be working on the mobile application.
...
### Setup a link to the UDLab/HexaDot repository

Before making changes to your repository, you will want to setup a reference back to the UDLab HexaDot repository so that you can pull changes into your forked repository.

1. Open the command prompt and navigate to the HexaDot directory.
1. Run the following command to setup a reference called upstream to the UDLab/HexaDot repository.

        git remote add upstream https://github.com/UDLab/HexaDot.git
...
In the HexaDotApp directory are 2 solution files.  You will want to open the **HexaDotWebOnly.sln**

![HexaDotWebOnly.sln file](images/web-1-sln-file.png)

1. Double-click on the "HexaDotWebOnly.sln" file in Windows Explorer
...
By default the Web project is set as the Startup Project.  You can verify this in Solution Explorer by looking at the Web\HexaDot project and seeing that it is bolded.
...
1. There are 2 accounts already created for you to log in with to test out different functionality.  You can get the account information at [https://github.com/UDLab/HexaDot/wiki/Solution-architecture#run-and-test-locally](https://github.com/UDLab/HexaDot/wiki/Solution-architecture#run-and-test-locally)
...
We need to verify that you can run the unit test.  HexaDot has over 1,000 unit test.  The expectation is that as you add functionality there is a unit test that accompanies it.  You should also run all of the unit test before submitting any pull arequest.

The unit test project is in the Test folder and called HexaDot.UnitTest.  
...
There is also a suite of UI test in the  Test\HexaDot.ScenarioTest project.  The test use  [Canopy](http://lefthandedgoat.github.io/canopy/index.html)which is an F# library that makes it easier to work with the Selenium Web Driver to automated web site UI tests.   
...
The HexaDot.ScenarioTest project is a console application.  You can either run it within Visual Studio or from the command line.  The instructions below are going to show how to run the tests from within Visual Studio.  


To run the UI test, you first need to start up the web site.

1.  The canopy tests require that the web site is running.  Make sure that the  Web\HexaDot project is the start up project.  Press F5 to start the web site in debug mode or Ctrl+F5 to start without debug mode.
...
1. Now that the web site is running and breakpoint is set, we are ready to run our UI test.  In Visual Studio solution explorer, right-click on the HexaDot.ScenarioTest project, select the Debug menu, and click on "Start New Instance"
...
You are now ready to start working on the HexaDot web site and unit test.  Please read through the [Git guide](../git/gitprocess.md) for the process of contributing to the HexaDot repository.
...
1. Navigate to the HexaDot\HexaDotApp\Mobile-App directory
...
In the HexaDotApp directory are 2 solution files.  You will want to open the **HexaDotApp.sln**

1. Double-click on the "HexaDotApp.sln" file in Windows Explorer to open it in Visual Studio 2015
...
Before launching the ionic npm task, we need to have the web site running in order to access the HexaDot Api. 

1. Set the Web\HexaDot project as the start up project by right clicking on the Web\HexaDot project and selecting "Set as Startup Project"
...
> If you want to download additional devices, open up the Visual Studio Emulator for Android, find the device that you want to download and click on the ![download device icon](images/emulator-2-download-icon.png) icon for the device you want.  Once you download a device it will automatically become available as a target in Visual Studio for the HexaDotApp project.


**WARNING:** With the default configuration of the Mobile App, you will not be able to communicate with the Api since it uses the web site running locally on your running using localhost which is not available to external devices or emulators.  To get around this you need to make sure that any Api changes you have made have have been submitted as a Pull Request and merged into the HexaDot master branch.  
...
To build and deploy the Mobile Application to the Visual Studio Emulator for Android you need to set the Mobile\HexaDotApp project to the startup project by bight clicking on the HexaDotApp project and selecting "Set as StartUp Project"

![set HexaDotApp as startup project](images/mobile-5-app-startup-set.png)
...
1. The emulator needs to be able to connect to the internet in order to talk with the HexaDot Api.  If you are prompted to configure the emulator to connect to the Internet, click Yes.
...
To build and deploy the Mobile Application to your Android device you need to set the Mobile\HexaDotApp project to the startup project by bight clicking on the HexaDotApp project and selecting "Set as StartUp Project"
